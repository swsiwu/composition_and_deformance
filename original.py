...
def generate_poem_img(lines, dalle_img_output_dir):
    ...
    if prompt != '':
        #counter += 1  #### this is problematic! remove this line!
        line_out_img_path = dalle_img_output_dir + str(counter) + '/' 
        if not os.path.exists(line_out_img_path):
            os.makedirs(line_out_img_path)
...
img_output_dir = './img_result/cornell_newsroom/original/' 
output_text_dir = './dataset/cornell_newsroom/output_text/original/'
poem_stats_path = './stats/cornell_newsroom/original/' +  'clip_stats' + '.csv'
...
with open(caption_file, newline='\n') as txtfile:
    ...
    for line in caption_lines:
        ...
        clip_scores, ave_clip = generate_poem_img(lines, dalle_img_output_dir)
        #output_poem(edited_output_text_dir, str(counter), edited_lines)
