...
def randomize_line(line):
    tokens = line.split(" ")
    random.shuffle(tokens)
    randomized_line = " ".join(tokens)  
...
def generate_poem_img(lines, dalle_img_output_dir):
    ...
    for line in lines:
        counter += 1
        if counter % 2 == 0:
            permuted_line = randomize_line(line)
            prompt = prompt + ' ' + permuted_line
...
img_output_dir = './img_result/cornell_newsroom/permuted/' 
output_text_dir = './dataset/cornell_newsroom/output_text/permuted/'
poem_stats_path = './stats/cornell_newsroom/permuted/'  + 'clip_stats' + '.csv'
