...
cond_scale = 3.0
# create a random key
def get_imageability_dict():
    """
    using MRC psycholinguistic database(Wilson 1988)
    https://websites.psychology.uwa.edu.au/school/mrcdatabase/uwa_mrc.htm


    key, word,mrc.nlet,mrc.phon,mrc.syl,mrc.fam,mrc.conc,mrc.imag,mrc.wtype
    rallies7020290,rallies,7,NA,NA,NA,NA,NA,

    #total 9240 words with img rating
    """
    database = './dataset/kao_jurafsky_dataset/wilson/MRC_corpus.csv'
    word_dict = defaultdict(float)

    with open(database, newline='\n') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
        for row in spamreader:
            if row[7] != 'NA' and row[7] != 'mrc.imag':
                word = row[1]
                word = regex.sub('', word)
                word = word.lower()
                word_dict[word] = row[7]


    return word_dict

def get_img_dict_with_score_as_key():
    img_dict = get_imageability_dict()
    score_dict = defaultdict(list)
    for word in img_dict.keys():
        score = img_dict[word]
        score_dict[str(score)].append(word)
    return score_dict
...
def replace_same_img_score_noun(line):  
    single_punct = [',', ';', '-', '.', '!', '–', '?' ,'—']
    double_punct = ['--', '––']
    ...
    replaced_nouns = []
    for noun in nouns:
        if noun.lower() in img_dict.keys():
            word_score = img_dict[noun.lower()]
            replaced_word_options = score_dict[str(word_score)]
            replaced_word = random.choice(replaced_word_options)
            
            replaced_nouns.append(replaced_word)
    ...
    for word in words:
        if len(word) >= 1:
            if word in double_punct:
                new_words.append(word)
                continue
            if word in single_punct:
                new_words.append(word)
                continue


            upper_tf = word[0].isupper()

            if word in nouns:
                for index, noun in enumerate(nouns):
                    if word == noun:
                        new_word = replaced_nouns[index]
                        if upper_tf == True:
...
def generate_poem_img(lines, dalle_img_output_dir):
    ...
    for line in lines:
        counter += 1
        if counter % 2 == 0:
            replaced_noun_line = replace_same_img_score_noun(line)
            prompt = prompt + ' ' + replaced_noun_line
...
img_dict = get_imageability_dict()
score_dict = get_img_dict_with_score_as_key()
